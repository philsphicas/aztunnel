...
// duplicate (backend, axis, scenario, mode) keys so a stale or
// double-appended artifact fails loudly instead of silently picking one
// row. It returns the data rows and the run-header records (the latter
// drive the provenance line).
		...
		for sc.Scan() {
			...
			if seen[key] {
				return nil, nil, fmt.Errorf("duplicate row for backend=%q axis=%q scenario=%q mode=%q run=%q "+
					"(stale or double-appended artifact?)", rec.Backend, canonicalAxes(rec), rec.Scenario, rec.Mode, rec.Run)
			}
			seen[key] = true
...
func renderProvenance(w io.Writer, runs []runMeta) {
	...
	fmt.Fprintln(w, "sources: "+strings.Join(parts, "  |  "))
	if len(shas) > 1 {
		fmt.Fprintln(w, "warning: merged artifacts span multiple builds (git shas differ) — comparison may mix versions")
	}
	fmt.Fprintln(w)
}
...
func renderTable(w io.Writer, recs []record) error {
	...
	fmt.Fprintln(w, "PERF MATRIX (client-side RTT; est = cold_p50 − warm_p50 ≈ establishment cost)")
...
func renderCompare(w io.Writer, recs []record, dim, baseSel, candSel string) (gateStats, error) {
	...
	if excluded > 0 {
		fmt.Fprintf(w, "warning: %d rows lack dimension %q and were excluded from the comparison\n", excluded, dim)
	}
	...
	fmt.Fprintf(w, "PERF COMPARE  %s: baseline=%s  candidate=%s  (Δ%% = (candidate−baseline)/baseline; negative is faster)\n", dim, base, cand)
	tw := tabwriter.NewWriter(w, 0, 0, 2, ' ', 0)
	var header []string
	if showBackend {
...
func warnMixedAxisKeys(w io.Writer, recs []record, cols []string) {
	...
	for _, r := range recs {
		for _, k := range cols {
			if _, ok := r.Axes[k]; !ok {
				fmt.Fprintf(w, "warning: rows carry differing axis dimensions (some lack %q) — filters/pivots on it will skip them\n", k)
				return
			}
		}
	}
...
func renderGrid(w io.Writer, recs []record, metric string) error {
	...
	fmt.Fprintf(w, "PERF MATRIX GRID  backend=%s  metric=%s  scenario=%s  mode=%s  (rows=sender, cols=listener; %s)\n",
