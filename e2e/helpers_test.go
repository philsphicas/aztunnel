...
// requireRelayEnv reads config from env vars and skips if not configured.
func requireRelayEnv(t *testing.T) *relayEnv {
	t.Helper()
	relay := os.Getenv("AZTUNNEL_RELAY_NAME")
	hyco := os.Getenv("AZTUNNEL_HYCO_NAME")
	if relay == "" || hyco == "" {
		t.Skip("AZTUNNEL_RELAY_NAME and AZTUNNEL_HYCO_NAME must be set for e2e tests")
	}
	return &relayEnv{
		relayName:          relay,
		hyco:               hyco,
		sasHyco:            os.Getenv("AZTUNNEL_SAS_HYCO_NAME"),
		sasListenerKeyName: os.Getenv("AZTUNNEL_SAS_LISTENER_KEY_NAME"),
		sasListenerKey:     os.Getenv("AZTUNNEL_SAS_LISTENER_KEY"),
		sasSenderKeyName:   os.Getenv("AZTUNNEL_SAS_SENDER_KEY_NAME"),
		sasSenderKey:       os.Getenv("AZTUNNEL_SAS_SENDER_KEY"),
	}
}
