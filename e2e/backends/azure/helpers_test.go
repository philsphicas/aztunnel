...
// repoRoot walks up from this file's source location until it finds
// a directory containing go.mod whose module is the aztunnel root.
// Used to anchor paths like `e2e/.local.json` and `bin/aztunnel`
// that the test process needs regardless of cwd. Robust to deep
// package directories (this package lives four levels below the
// repo root) and to running individual tests through `go test -run`
// from any cwd.
...
// because scenarios.AssertNoLeaks samples process-wide goroutine
...
//
// Backend contract (for callers wiring this into an scenarios.Backend
