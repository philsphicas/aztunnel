...
//   - AZTUNNEL_HYCO_NAME: hybrid connection name (Entra ID auth)
//   - Valid Azure credentials (az login, managed identity, etc.)
//
// Optional:
//
//   - AZTUNNEL_SAS_HYCO_NAME: hybrid connection for SAS key tests
//   - AZTUNNEL_SAS_LISTENER_KEY_NAME + AZTUNNEL_SAS_LISTENER_KEY: listener SAS credentials
//   - AZTUNNEL_SAS_SENDER_KEY_NAME + AZTUNNEL_SAS_SENDER_KEY: sender SAS credentials
...
func TestPortForwardBasic(t *testing.T) {
	...
	listener := startAztunnel(t, env,
		"relay-listener", env.hyco,
		"--relay", env.relayName,
		"--allow", echo.Addr(),
		"--metrics-addr", ":0",
		"--log-level", "debug",
	)
	sender := startAztunnel(t, env,
...
func TestSASKeyAuth(t *testing.T) {
	env := requireRelayEnv(t)
	if env.sasHyco == "" || env.sasListenerKeyName == "" || env.sasSenderKeyName == "" {
		t.Skip("SAS credentials not configured (AZTUNNEL_SAS_HYCO_NAME, AZTUNNEL_SAS_LISTENER_KEY_NAME, AZTUNNEL_SAS_SENDER_KEY_NAME)")
	}
...
func TestConcurrentDistinctTargets(t *testing.T) {
	...
	listener := startAztunnel(t, env, args...)
...
func TestBadRelayName(t *testing.T) {
	...
	proc := startAztunnel(t, env,
		"relay-listener", env.hyco,
		"--relay", "nonexistent-relay-xxxxx",
	)
...
func TestBadHycoName(t *testing.T) {
	...
	proc := startAztunnel(t, env,
		"relay-listener", "nonexistent-hyco-xxxxx",
		"--relay", env.relayName,
	)
...
func TestPortForwardClosedPort(t *testing.T) {
	...
	sender := startAztunnel(t, env,
		"relay-sender", "port-forward", closedAddr,
		"--relay", env.relayName,
		"--hyco", env.hyco,
		"--bind", "127.0.0.1:0",
		"--log-level", "debug",
...
func TestPortForwardUnreachable(t *testing.T) {
	...
	sender := startAztunnel(t, env,
		"relay-sender", "port-forward", unreachable,
		"--relay", env.relayName,
		"--hyco", env.hyco,
		"--bind", "127.0.0.1:0",
		"--log-level", "debug",
