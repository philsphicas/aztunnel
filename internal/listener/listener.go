...
type Config struct {
	...
	// RejectMux, when true, causes the listener to respond to a v2
	// MuxHandshake first message with a v1-style "unsupported protocol
	// version" rejection instead of dispatching to handleMuxSession.
	// This is a test seam used by the rolling-deployment scenario
	// (sender-v2 against listener-v1) and by Azure rolling-upgrade
	// simulations. Production callers leave this false. The rejection
	// string is one of the values isMuxUnsupportedRejection in
	// internal/sender/muxdialer.go recognises, so the sender falls
	// back to v1 cleanly and caches the fallback for muxUnavailableTTL.
	RejectMux bool
...
func handleConnection(ctx context.Context, ws *websocket.Conn, cfg Config, streamSem, pendingSem *connSemaphore) {
	...
	if first.IsMux() {
		if cfg.RejectMux {
			logger.Warn("rejecting mux handshake per Config.RejectMux (test seam)")
