...
#
# The `[^/]+/` segment skips the backend axis layer (auth cell, e.g.
# sas/entra) that the mock and Azure backends insert above the
# scenario name under default env. (Pinning a single axis value,
# e.g. E2E_AUTH=sas, collapses that layer — pass a matching PERF
# regex or run scenarios directly if you do that.)
...
perf-clean-history: ## Delete the accumulated per-run history files
	rm -f e2e/$(PERF_HISTORY_DIR)/*.jsonl
